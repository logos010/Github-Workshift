...
foreach($profile->loadProfileFields() as $field) {
	echo CHtml::openTag('p',array('class'=>'nomt'));

	if($field->hint)
		echo CHtml::tag('div',array('class'=>'hint'),$field->hint,true);

	echo CHtml::activeLabelEx($profile, $field->varname); echo '<br/>';
	if ($field->field_type=='BOOLEAN') {
		...
		echo CHtml::activeTextField($profile,
				$field->varname,
				array('size'=>(($field->field_size_min)?$field->field_size_min:40),'maxlength'=>(($field->field_size)?$field->field_size:255), 'class' => 'input-text'));
	}
