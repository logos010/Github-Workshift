...
//        $this->layout = Yum::module()->loginLayout;
        ...
        if (isset($_POST['YumUserLogin'])) {
            ...
            if ($success instanceof YumUser) {
                ...
                Yum::log(Yum::t('User {username} successfully logged in (Ip: {ip})', array(
                            '{ip}' => Yii::app()->request->getUserHostAddress(),
...
//                $this->redirectUser($success);
    ...
    /**
     * get userid through username
     */
    public function getUserIdByName($username) {
        $userModel = new YumUser;
        $user = $userModel->find('username=:username', array(
            ':username' => $username
                ));
     ...
     * User login this application for the attendence in working days.
     * Each day, the laboure must access 4 times 
     */
    public function labourerAttend($username, $type='IN') {
        $userID = $this->getUserIdByName($username);

        //if user already checked in at morning, skip insert
        $checkin = CheckinDairy::model()->findByAttributes(array(
            'user_id' => $userID,
            'checkin_date' => date('d-m-Y', time()),
                ), 'session=:session', array(
            ':session' => ServiceUtil::getTimeSession()
                ));

        //skip insert checkin_dairy table if user already checked in for morning or afternoon session
        if ($checkin === null) {
            $dairy = new CheckinDairy();
            $dairy->user_id = $this->getUserIdByName($username);
            $dairy->checkin_date = date('d-m-Y', time());
            if ($type == 'IN')
                $dairy->start_time = time();
            $dairy->session = ServiceUtil::getTimeSession();

            if (date('N', time()) >= 6) { // 6: Saturday, 7: Sunday
                $dairy->holiday_type = WEEKEND;
            } else if (date('N', time()) >= 6 && ServiceUtil::isHoliday($dairy->checkin_date)) {
                $dairy->holiday_type = PUBLIC_HOLIDAY;
            } else {
                $dairy->holiday_type = 0;
            }
    ...
    public function getTimeSession() {
        if (((int) date('H', time())) < 12) {
            return 'MORNING';
        } elseif (((int) date('H', time())) > 12 && ((int) date('H', time())) < 18) {
            return 'AFTERNOON';
        }
    }
    ...
    public function actionLogout() {
        ...
        if ($user = YumUser::model()->findByPk(Yii::app()->user->id)) {
            ...
            //checkout user into checkin_dairy table
            $checkout = CheckinDairy::model()->findByAttributes(array(
            ...
            if ($checkout !== null)
                $checkout->update(array('end_time'));
